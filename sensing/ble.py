...
import numpy as np
import json
from bluepy.btle import Scanner, DefaultDelegate, Peripheral, UUID, ADDR_TYPE_PUBLIC
import time
import csv
...
class MyDelegate(DefaultDelegate):
    ...
    def handleNotification(self, cHandle, data):
        global last_msg, last_quality_msg, last_beat_msg, sensorType, ProtoDevice
        ...
        for (adtype, desc, value) in dev.getScanData():
            ...
            if desc == "Complete Local Name":
                if value == "Proto-Fever" or value == "Proto-Light" or value == "Proto-Beat" or value == "Proto-Quality":
                    sensorType = value
                    log.debug("Device %s (%s), RSSI=%d dB" % (dev.addr, dev.addrType, dev.rssi))
                    device = Peripheral(dev.addr, ADDR_TYPE_PUBLIC)
                    device.setMTU(60)
